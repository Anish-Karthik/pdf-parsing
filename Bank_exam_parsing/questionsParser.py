...
def remove_question_number(text):
    return re.sub(r"^Q\d+\.", "", text)


def extract_question_number(text):
    qno = re.findall(r"^Q(\d+)\.", text)
...
def get_each_lines(doc, isAnswer=False):
    ...
    for page in doc:
        ...
        pg_lines = [list(block) + [True] for block in pg_lines]
...
def get_questions_alter(lines) -> List[Question]:
    ...
    options_length = 5
    for ind, line in enumerate(lines):
        if cur_op == options_length - 1 and not is_part_of_last_option(lines[ind - 1], line):
            ...
            op_0_ind = None
            cur_op = 0
        if (cur_op < options_length - 1 and is_option_match(cur_op + 1, line[4])):
            cur_op += 1
            options.append(Option(clean_text(remove_option_number(op_text)).replace(" ... ", " . . . ")))
