...
def create_reading_material(json_path, ncert_sentence_wise_embeddings):

    quiz = read_json_file(json_path)

    call_collection_with_threading(
        func=create_question_material_from_search,
        args=(ncert_sentence_wise_embeddings,),
        threads=10,
...
for quiz_id in quiz_id_to_pdf_map:
    ...
    create_reading_material(json_path, ncert_sentence_wise_embeddings)
